...
unsigned long long cruise_learn_update_total = 0;
unsigned long long cruise_learn_update_count = 0;
uint64_t cruise_learn_update_t1;
uint64_t cruise_learn_update_t2;
TimingLogger cruise_learn_update_logger("cruise_learn_update");
...
{
    cruise_learn_update_t1 = AP_WCET::now_us();
    float speed;
    ...
    cruise_learn_update_t2 = AP_WCET::now_us();
    uint64_t duration = cruise_learn_update_t2 - cruise_learn_update_t1;
    cruise_learn_update_total += duration;
    cruise_learn_update_count += 1;
    
    cruise_learn_update_logger.log(duration);
    if (cruise_learn_update_count % 1000 == 0){
        printf("average cruise_learn_update measure time microseconds is %f!\n",cruise_learn_update_total/1000.0);
        cruise_learn_update_total = 0;
        cruise_learn_update_count = 0; 
    }
    return;
}
