...
unsigned long long ahrs_update_total = 0;
unsigned long long ahrs_update_count = 0;
uint64_t ahrs_update_t1;
uint64_t ahrs_update_t2;
TimingLogger ahrs_update_logger("ahrs_update");


unsigned long long update_current_mode_total = 0;
unsigned long long update_current_mode_count = 0;
uint64_t update_current_mode_t1;
uint64_t update_current_mode_t2;
TimingLogger update_current_mode_logger("update_current_mode");

unsigned long long update_logging1_total = 0;
unsigned long long update_logging1_count = 0;
uint64_t update_logging1_t1;
uint64_t update_logging1_t2;
TimingLogger update_logging1_logger("update_logging1");

unsigned long long update_logging2_total = 0;
unsigned long long update_logging2_count = 0;
uint64_t update_logging2_t1;
uint64_t update_logging2_t2;
TimingLogger update_logging2_logger("update_logging2");

unsigned long long gcs_failsafe_check_total = 0;
unsigned long long gcs_failsafe_check_count = 0;
uint64_t gcs_failsafe_check_t1;
uint64_t gcs_failsafe_check_t2;
TimingLogger gcs_failsafe_check_logger("gcs_failsafe_check");

unsigned long long one_second_loop_total = 0;
unsigned long long one_second_loop_count = 0;
uint64_t one_second_loop_t1;
uint64_t one_second_loop_t2;
TimingLogger one_second_loop_logger("one_second_loop");

unsigned long long stats_update_total = 0;
unsigned long long stats_update_count = 0;
uint64_t stats_update_t1;
uint64_t stats_update_t2;
TimingLogger stats_update_logger("stats_update");
}
...
{
    stats_update_t1 = AP_WCET::now_us();

    g2.stats.set_flying(g2.motors.active());
    g2.stats.update();

    stats_update_t2 = AP_WCET::now_us();
    uint64_t duration = stats_update_t2 - stats_update_t1;
    stats_update_total += duration;
    stats_update_count += 1;
    
    stats_update_logger.log(duration);
    if (stats_update_count % 1000 == 0){
        printf("average stats_update measure time microseconds is %f!\n",stats_update_total/1000.0);
        stats_update_total = 0;
...
void Rover::ahrs_update()
{
    ...
    ahrs_update_t2 = AP_WCET::now_us();
    uint64_t duration = ahrs_update_t2 - ahrs_update_t1;
    ahrs_update_total += duration;
    ahrs_update_count += 1;
    
    ahrs_update_logger.log(duration);
    if (ahrs_update_count % 1000 == 0){
        printf("average ahrs_update measure time microseconds is %f!\n",ahrs_update_total/1000.0);
        ahrs_update_total = 0;
        ahrs_update_count = 0; 
    }
}
...
{
    gcs_failsafe_check_t1 = AP_WCET::now_us();
    if (!g.fs_gcs_enabled) {
    ...
    gcs_failsafe_check_t2 = AP_WCET::now_us();
    uint64_t duration = gcs_failsafe_check_t2 - gcs_failsafe_check_t1;
    gcs_failsafe_check_total += duration;
    gcs_failsafe_check_count += 1;
    
    gcs_failsafe_check_logger.log(duration);
    if (gcs_failsafe_check_count % 1000 == 0){
        printf("average gcs_failsafe_check measure time microseconds is %f!\n",gcs_failsafe_check_total/1000.0);
        gcs_failsafe_check_total = 0;
        gcs_failsafe_check_count = 0; 
    }
}
...
{
    update_logging1_t1 = AP_WCET::now_us();
    ...
    update_logging1_t2 = AP_WCET::now_us();
    uint64_t duration = update_logging1_t2 - update_logging1_t1;
    update_logging1_total += duration;
    update_logging1_count += 1;
    
    update_logging1_logger.log(duration);
    if (update_logging1_count % 1000 == 0){
        printf("average update_logging1 measure time microseconds is %f!\n",update_logging1_total/1000.0);
        update_logging1_total = 0;
        update_logging1_count = 0; 
    }
}
...
{
    update_logging2_t1 = AP_WCET::now_us();
    ...
    update_logging2_t2 = AP_WCET::now_us();
    uint64_t duration = update_logging2_t2 - update_logging2_t1;
    update_logging2_total += duration;
    update_logging2_count += 1;
    
    update_logging2_logger.log(duration);
    if (update_logging2_count % 1000 == 0){
        printf("average update_logging2 measure time microseconds is %f!\n",update_logging2_total/1000.0);
        update_logging2_total = 0;
        update_logging2_count = 0; 
    }
}
...
{
    one_second_loop_t1 = AP_WCET::now_us();
    ...
    one_second_loop_t2 = AP_WCET::now_us();
    uint64_t duration = one_second_loop_t2 - one_second_loop_t1;
    one_second_loop_total += duration;
    one_second_loop_count += 1;
    
    one_second_loop_logger.log(duration);
    if (one_second_loop_count % 1000 == 0){
        printf("average one_second_loop measure time microseconds is %f!\n",one_second_loop_total/1000.0);
        one_second_loop_total = 0;
        one_second_loop_count = 0; 
    }
}

void Rover::update_current_mode(void)
{
    update_current_mode_t1 = AP_WCET::now_us();
    ...
    update_current_mode_t2 = AP_WCET::now_us();
    uint64_t duration = update_current_mode_t2 - update_current_mode_t1;
    update_current_mode_total += duration;
    update_current_mode_count += 1;
    
    update_current_mode_logger.log(duration);
    if (update_current_mode_count % 1000 == 0){
        printf("average update_current_mode measure time microseconds is %f!\n",update_current_mode_total/1000.0);
        update_current_mode_total = 0;
        update_current_mode_count = 0; 
    }
}
