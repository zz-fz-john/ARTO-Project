...
unsigned long long read_rangefinders_total = 0;
unsigned long long read_rangefinders_count = 0;
uint64_t read_rangefinders_t1;
uint64_t read_rangefinders_t2;
TimingLogger read_rangefinders_logger("read_rangefinders");


unsigned long long update_wheel_encoder_total = 0;
unsigned long long update_wheel_encoder_count = 0;
uint64_t update_wheel_encoder_t1;
uint64_t update_wheel_encoder_t2;
TimingLogger update_wheel_encoder_logger("update_wheel_encoder");

unsigned long long update_compass_total = 0;
unsigned long long update_compass_count = 0;
uint64_t update_compass_t1;
uint64_t update_compass_t2;
TimingLogger update_compass_logger("update_compass");
}
// check for new compass data - 10Hz
void Rover::update_compass(void)
{   
    update_compass_t1 = AP_WCET::now_us();
    compass.read();
    update_compass_t2 = AP_WCET::now_us();
    uint64_t duration = update_compass_t2 - update_compass_t1;
    update_compass_total += duration;
    update_compass_count += 1;
    
    update_compass_logger.log(duration);
    if (update_compass_count % 1000 == 0){
        printf("average update_compass measure time microseconds is %f!\n",update_compass_total/1000.0);
        update_compass_total = 0;
        update_compass_count = 0;
    }
}
...
{
    update_wheel_encoder_t1 = AP_WCET::now_us();
...
#endif
    update_wheel_encoder_t2 = AP_WCET::now_us();
    uint64_t duration = update_wheel_encoder_t2 - update_wheel_encoder_t1;
    update_wheel_encoder_total += duration;
    update_wheel_encoder_count += 1;
    
    update_wheel_encoder_logger.log(duration);
    if (update_wheel_encoder_count % 1000 == 0){
        printf("average update_wheel_encoder measure time microseconds is %f!\n",update_wheel_encoder_total/1000.0);
        update_wheel_encoder_total = 0;
        update_wheel_encoder_count = 0;
    }
}
...
{
    read_rangefinders_t1 = AP_WCET::now_us();
    rangefinder.update();
    Log_Write_Depth();
    read_rangefinders_t2 = AP_WCET::now_us();
    uint64_t duration = read_rangefinders_t2 - read_rangefinders_t1;
    read_rangefinders_total += duration;
    read_rangefinders_count += 1;
    
    read_rangefinders_logger.log(duration);
    if (read_rangefinders_count % 1000 == 0){
        printf("average read_rangefinders measure time microseconds is %f!\n",read_rangefinders_total/1000.0);
        read_rangefinders_total = 0;
        read_rangefinders_count = 0;
    }
}
