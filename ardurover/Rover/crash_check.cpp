...
unsigned long long crash_check_total = 0;
unsigned long long crash_check_count = 0;
uint64_t crash_check_t1;
uint64_t crash_check_t2;
TimingLogger crash_check_logger("crash_check");
...
void Rover::crash_check()
{
    ...
    crash_check_t2 = AP_WCET::now_us();
    uint64_t duration = crash_check_t2 - crash_check_t1;
    crash_check_total += duration;
    crash_check_count += 1;
    
    crash_check_logger.log(duration);
    if (crash_check_count % 1000 == 0){
        printf("average crash_check measure time microseconds is %f!\n",crash_check_total/1000.0);
        crash_check_total = 0;
        crash_check_count = 0; 
    }
}
