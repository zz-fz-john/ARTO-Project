...
unsigned long long read_radio_total = 0;
unsigned long long read_radio_count = 0;
uint64_t read_radio_t1;
uint64_t read_radio_t2;
TimingLogger read_radio_logger("read_radio");
...
void Rover::read_radio()
{
    ...
    read_radio_t2 = AP_WCET::now_us();
    uint64_t duration = read_radio_t2 - read_radio_t1;
    read_radio_total += duration;
    read_radio_count += 1;
    
    read_radio_logger.log(duration);
    if (read_radio_count % 1000 == 0){
        printf("average read_radio measure time microseconds is %f!\n",read_radio_total/1000.0);
        read_radio_total = 0;
    }
}
