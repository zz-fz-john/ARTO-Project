...
unsigned long long AP_GPS6update_total = 0;
unsigned long long AP_GPS6update_count = 0;
std::chrono::high_resolution_clock::time_point AP_GPS6update_t1;
std::chrono::high_resolution_clock::time_point AP_GPS6update_t2;
...
{
    AP_GPS6update_t1= std::chrono::high_resolution_clock::now();
    WITH_SEMAPHORE(rsem);
...
#endif
    AP_GPS6update_t2 = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(AP_GPS6update_t2 -AP_GPS6update_t1).count();
    AP_GPS6update_total += duration;
    AP_GPS6update_count += 1;
    if (AP_GPS6update_count % 100 == 0){
        printf("average AP_GPS6update measure time microseconds is %f!\n",AP_GPS6update_total/100.0);
        AP_GPS6update_total = 0;
    }
}
