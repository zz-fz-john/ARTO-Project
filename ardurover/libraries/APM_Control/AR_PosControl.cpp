...
void  AR_PosControl::input_pos_target(const Vector2p &pos, float dt)
{
    ...
    Vector2f __attribute__((annotate("nova_sensitive_var"))) vel;
    Vector2f __attribute__((annotate("nova_sensitive_var"))) accel;
    const float __attribute__((annotate("nova_sensitive_var"))) accel_max = MIN(_accel_max, _lat_accel_max);
