...
std::chrono::high_resolution_clock::time_point read_aux_all_t1;
std::chrono::high_resolution_clock::time_point read_aux_all_t2;

unsigned long long read_mode_switch_total = 0;
unsigned long long read_mode_switch_count = 0;
std::chrono::high_resolution_clock::time_point read_mode_switch_t1;
std::chrono::high_resolution_clock::time_point read_mode_switch_t2;
}
/*
...
{
    read_aux_all_t1= std::chrono::high_resolution_clock::now();
    if (!has_valid_input()) {
    ...
    read_aux_all_t2 = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(read_aux_all_t2 -read_aux_all_t1).count();
    read_aux_all_total += duration;
    read_aux_all_count += 1;
    if (read_aux_all_count % 100 == 0){
        printf("average read_aux_all measure time microseconds is %f!\n",read_aux_all_total/100.0);
...
{
    read_mode_switch_t1= std::chrono::high_resolution_clock::now();
    if (!has_valid_input()) {
        // exit immediately when no RC input
        return;
    }
    ...
    c->read_mode_switch();
    read_mode_switch_t2 = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(read_mode_switch_t2 -read_mode_switch_t1).count();
    read_mode_switch_total += duration;
    read_mode_switch_count += 1;
    if (read_mode_switch_count % 100 == 0){
        printf("average read_mode_switch measure time microseconds is %f!\n",read_mode_switch_total/100.0);
