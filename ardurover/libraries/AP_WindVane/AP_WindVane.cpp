...
unsigned long long AP_WindVane_update_total = 0;
unsigned long long AP_WindVane_update_count = 0;
std::chrono::high_resolution_clock::time_point AP_WindVane_update_t1;
std::chrono::high_resolution_clock::time_point AP_WindVane_update_t2;
...
{
    AP_WindVane_update_t1= std::chrono::high_resolution_clock::now();
...
// @Field: SpdTru: True wind speed
    AP::logger().WriteStreaming("WIND", "TimeUS,DrRaw,DrApp,DrTru,SpdRaw,SpdApp,SpdTru",
                        ...
                        _speed_apparent,
                        _speed_true);
    AP_WindVane_update_t2 = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(AP_WindVane_update_t2 -AP_WindVane_update_t1).count();
    AP_WindVane_update_total += duration;
    AP_WindVane_update_count += 1;
    if (AP_WindVane_update_count % 100 == 0){
        printf("average AP_WindVane_update measure time microseconds is %f!\n",AP_WindVane_update_total/100.0);
        AP_WindVane_update_total = 0;
        AP_WindVane_update_count = 0; 
    }
