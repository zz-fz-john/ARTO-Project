...
unsigned long long periodic_total = 0;
unsigned long long periodic_count = 0;
std::chrono::high_resolution_clock::time_point periodic_t1;
std::chrono::high_resolution_clock::time_point periodic_t2;
...
{
    periodic_t1= std::chrono::high_resolution_clock::now();

    batchsampler.periodic();

    periodic_t2 = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(periodic_t2 -periodic_t1).count();
    periodic_total += duration;
    periodic_count += 1;
    if (periodic_count % 100 == 0){
        printf("average periodic measure time microseconds is %f!\n",periodic_total/100.0);
        periodic_total = 0;
        periodic_count = 0; 
    }
}
