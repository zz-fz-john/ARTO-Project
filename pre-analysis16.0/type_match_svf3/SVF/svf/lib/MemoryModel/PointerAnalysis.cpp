...
bool isTypeMachted(const llvm::CallBase* indirectcallsite,const llvm::Function * func ){
            ...
            ii != ie; ii++)
    {
        ...
        if(ObjVar* objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii)))
        {
            ...
            if(obj->isFunction())
            {
                ...
                {
                        const SVF::SVFInstruction *ind_callsite_svf=cs->getCallSite();
                        ...
                        const SVF::SVFFunction *target =callee;
                        ...
                        if(isTypeMachted(cBase,tartget_function)==false)
                            {
                            continue;
                            }
...
void PointerAnalysis::connectVCallToVFns(const CallICFGNode* cs, const VFunSet &vfns, CallEdgeMap& newEdges)
{
            ...
            feit = vfns.end(); fit != feit; ++fit)
    {
        ...
        if (getIndCallMap()[cs].count(callee) > 0)
            continue;
        if(SVFUtil::getSVFCallSite(cs->getCallSite()).arg_size() == callee->arg_size()&&(!(SVFUtil::getSVFCallSite(cs->getCallSite()).isVarArg()) && !(callee->isVarArg())))
        {
            const SVF::SVFInstruction *ind_callsite_svf=cs->getCallSite();
            const llvm::Value * ind_callsite_llvm=SVF::LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(ind_callsite_svf);
            const llvm::Instruction *ind_callsite=llvm::dyn_cast<const llvm::Instruction>(ind_callsite_llvm);
        ...
        if((SVFUtil::getSVFCallSite(cs->getCallSite()).isVarArg() && callee->isVarArg()))
        {
            newEdges[cs].insert(callee);
            getIndCallMap()[cs].insert(callee);
            const CallICFGNode* callBlockNode = pag->getICFG()->getCallICFGNode(cs->getCallSite());
