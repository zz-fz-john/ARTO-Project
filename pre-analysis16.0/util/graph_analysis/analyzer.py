...
def is_ignored_function(func_name):
    return func_name in IGNORE_FUNCTIONS

#旨在找到图 R 中可以以最低成本合并的一对数据区域。该函数接受两个参数：表示图的 R 和当前感兴趣的区域 c_region。
...
def add_region_to_comp_desc(R,r_node,comp_desc):
    ...
    policy_r_type = r_node_to_comp_lut[r_type]#data or code
    # print r_node
    # print ("--------")
    # print OBJECTS_KEY
...
def add_region_node(R,name,r_type,r_id=0,objects=None,attrs=None,merge=None,name_attr=None):
    ...
    if objects:
        ...
        #print(region_attrs[OBJECTS_KEY])
        #print("has objects")
    if attrs:
...
def get_call_subgraph(R, root_func):
    ...
    while stack:
        current = stack.pop()
        if is_ignored_function(current):
            continue
...
def partition_by_operation(G, T,key_operations):
    ...
    if "critical" not in operation_to_code_nodes:
        operation_to_code_nodes["critical"] = [] 
    ...
    if "critical" not in operation_to_code_nodes:
        operation_to_code_nodes["critical"] = []
    operation_to_code_nodes["critical"].extend(non_critical_funcs)
    operation_to_code_nodes["critical"].extend(shared_functions)
    ...
    for func in critical_func_after:
        for predecessor in Region_Graph.predecessors(func):
            ...
            if predecessor in  Region_Graph.node:
                node_data = Region_Graph.node[predecessor] 
    ...
    if "critical" not in operation_to_data_nodes:
        operation_to_data_nodes["critical"] = []
    operation_to_data_nodes["critical"].extend(non_critical_data)
    operation_to_data_nodes["critical"].extend(shared_code_data)
...
def AMI_build_data_regions_from_dict(R,region_dict,r_type,r_id=0):
    ...
    for key, nodes in region_dict.items():##key为段名后缀，nodes为全局变量的列表
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s','--sec_mask',dest='sec_mask', help='sec_mask_file')
    ...
    parser.add_argument('--ignore_func', dest='ignore_func', action='append', default=[],
                        help='Function name to ignore during DFS. Can be used multiple times, e.g. --ignore_func foo --ignore_func bar')
    parser.add_argument('--ignore_func_file', dest='ignore_func_file', default=None,
                        help='Path to a file containing function names to ignore, one per line')
    ...
    for func in args.ignore_func:
        for item in func.split(','):
            item = item.strip()
            if item:
                IGNORE_FUNCTIONS.add(item)
    if args.ignore_func_file:
        with open(args.ignore_func_file, 'r') as ignore_file:
            for line in ignore_file:
                item = line.strip()
                if item:
                    IGNORE_FUNCTIONS.add(item)
    print("Ignore function size is :" + str(len(IGNORE_FUNCTIONS)))
