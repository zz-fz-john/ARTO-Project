...
SHARED_DATA_SECTIONS_FILE = current_dir+"/../../../measurement-engine/first-measure-in-secure-world/runtime-measure/shared_data_sections.S"
...
def AMI_make_linker_script(template_linker, linker_out, sec_mask_file,partitions):#

    sect_addr_filename = sec_mask_file#add modify
    # wait for compartment start address
    if not os.path.isfile(sect_addr_filename):
        print("Error: section address file %s does not exist!" % sect_addr_filename)
        return
...
def AMI_get_sections_strings_from_partition(partitions):
    ...
    """
    1_code_section
    1_shared_data_section
    1_data_section
    2_code_section
    2_shared_data_section
    2_data_section

    """
    for r_id in sorted(partitions['Regions'].keys()):###POLICY_KEY_REGIONS = "Regions",r_id=r_node
        # print(r_id)
        region = partitions['Regions'][r_id]
        text_section = None
        data_section = None
        ...
        if region['Type'] == 'Code':
            ...
            end_index = r_id.find('_',13)#.CODE_REGION_rid_
            # reg_id = r_id[13]
            reg_id = r_id[13: end_index]#rid
            # print reg_id
