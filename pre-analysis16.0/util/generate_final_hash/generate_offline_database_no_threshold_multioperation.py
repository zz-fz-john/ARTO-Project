...
def load_critical_function_names(filepath):
    """
    从critical_function.txt读取函数名，忽略空行和注释行。
    """
    names=[]
...
def get_function_addr_ranges(func_map):
    """
    根据func_name_map构造函数地址区间列表: (func_name, start_addr, next_start_addr)
    """
    items=[]
    for name,addr in func_map.items():
        items.append((name,int(addr,16)))
    items.sort(key=lambda x:x[1])
    ranges=[]
    for i,(name,start_addr) in enumerate(items):
        next_addr=items[i+1][1] if i+1 < len(items) else None
        ranges.append((name,start_addr,next_addr))
    return ranges

def find_start_collecting_callsites(opts, critical_function_file):
    """
    在critical_function_file列出的关键函数体内，用正则在反汇编文本中查找
    调用collect_start(即start_collecting)的BL类指令地址。
    """
    if critical_function_file is None:
        return []
    if not os.path.isfile(critical_function_file):
        exit("%s: file '%s' not found" % (sys.argv[0], critical_function_file))

    critical_functions=load_critical_function_names(critical_function_file)
    if len(critical_functions) == 0:
        return []

    function_pattern=re.compile(r"^\s*([0-9A-Fa-f]+)\s+<([^>]+)>:\s*$")
    bl_pattern=re.compile(r"^\s*([0-9A-Fa-f]+):\s+.*\bbl\w*\s+([0-9A-Fa-fx]+)(?:\s+<([^>]+)>)?")

    critical_set=set(critical_functions)
    seen_functions=set()
    found_callsites={}
    current_function=None

    with open(opts.disassemble_file, 'r') as file:
        for raw_line in file:
            ...
            if func_match:
                ...
                if current_function in critical_set:
                    seen_functions.add(current_function)
                continue

            if current_function not in critical_set:
                continue
            if current_function in found_callsites:
                continue

            bl_match=bl_pattern.match(line)
            if not bl_match:
                continue

            inst_addr=bl_match.group(1)
            target_text=bl_match.group(2)
            target_symbol=bl_match.group(3) if bl_match.group(3) else ""

            is_collect_start=False
            try:
                target_addr=int(target_text,16)
                if target_addr == opts.collect_start:
                    is_collect_start=True
            except ValueError:
                is_collect_start=False

            if not is_collect_start and ("start_collecting" in target_symbol):
                is_collect_start=True

            if is_collect_start:
                found_callsites[current_function]=int(inst_addr,16)

    for func_name in critical_functions:
        if func_name not in seen_functions:
            logging.warning("critical function '%s' not found in disassembly file", func_name)
        elif func_name not in found_callsites:
            logging.warning("start_collecting call not found in critical function '%s'", func_name)

    callsites=[]
    for func_name in critical_functions:
        if func_name in found_callsites:
            callsites.append(found_callsites[func_name])
    return callsites
...
def main():
    ...
    parser.add_argument('-cff','--critical_function_file',dest='critical_function_file',default=None,
            help='critical_function file path')
    ...
    opts=Namespace(
            ...
            only_called_once_file   =   args.only_called_once_func_file,
                critical_function_file  =   args.critical_function_file,
    ...
    logging.debug("critical_function_file       = %s" % opts.critical_function_file)
    ...
    for i in md.disasm(code,current_address):
        ...
        if i.id==ARM_INS_BL:#direct call bl
            ...
            if target_address in instrument_func_addr:
                ...
                fake_stack=[]
                print("encounter start_collecting")
...
def analysis(opts):##使用capstone工具
    ...
    target_address=0
    global compute_hash
    start_addresses=find_start_collecting_callsites(opts,opts.critical_function_file)
    if len(start_addresses) == 0:
        start_addresses=[opts.text_start]
    print("start addresses are:")
    for addr in start_addresses:
        print(hex(addr))
    for start_addr in start_addresses:
        print("start address is "+hex(start_addr))
        with open(opts.binfile,"rb") as f:
            mm=mmap.mmap(f.fileno(),0,prot=mmap.PROT_READ)
            opts.text_start=start_addr
            ...
            branch_list=[]
            global hash_map
            recurse_to_hash(opts,mm,md,current_address,None,edge_list,[],0,0,branch_list)
