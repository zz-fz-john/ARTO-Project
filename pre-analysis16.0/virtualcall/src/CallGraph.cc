...
void CallGraphPass::PrintResults(CallInst *CI, FuncSet FS, std::string MLTypeName) {
	...
	if(!incallID){
		LLVM_ERROR("No inCallID metadata found for callsite: " << CI);
		exit(1);
	}
	...
	errs() <<"In function : "<<CI->getParent()->getParent()->getName().str()<<"  ";
	errs() << "indirect callsite :   " << ID << "\n";
	if (FS.empty()){
		errs() << "No target." << "\n";
		...
		while(!FS.empty()){
			...
			if(CurFunc->getName().str().find(".")==std::string::npos){//去除掉对形如" _GLOBAL__sub_I_lua_scripts.cpp"的调用
				vCallCandidates.insert(CurFunc);
			}
			FS.erase(CurFunc);
		}
		CallBase *CB=dyn_cast<CallBase>(CI);
		incallEdges_SMLTA.emplace(ID, incall::IndirectCallEdge(*CB,true, vCallCandidates));
