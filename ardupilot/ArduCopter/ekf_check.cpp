...
uint64_t ekf_check_t1;
uint64_t ekf_check_t2;
TimingLogger ekf_check_logger("ekf_check");

unsigned long long check_ekf_reset_total = 0;
unsigned long long check_ekf_reset_count = 0;
uint64_t check_ekf_reset_t1;
uint64_t check_ekf_reset_t2;
TimingLogger check_ekf_reset_logger("check_ekf_reset");
...
void Copter::ekf_check()
{
    ...
    ekf_check_t2 = AP_WCET::now_us();
    uint64_t duration = ekf_check_t2 -ekf_check_t1;
    ekf_check_total += duration;
    ekf_check_count += 1;
    if (ekf_check_count % 1000 == 0){
       // printf("average ekf_check measure time microseconds is %f!\n",ekf_check_total/1000.0);
        ekf_check_total = 0;
        ekf_check_count = 0;
    }
    ekf_check_logger.log(duration);
}
...
{
    check_ekf_reset_t1 = AP_WCET::now_us();
    // check for yaw reset
    ...
    check_ekf_reset_t2 = AP_WCET::now_us();
    uint64_t duration = check_ekf_reset_t2 - check_ekf_reset_t1;
    check_ekf_reset_total += duration;
    check_ekf_reset_count += 1;
    if (check_ekf_reset_count % 1000 == 0){
        //printf("average check_ekf_reset measure time microseconds is %f!\n", check_ekf_reset_total/1000.0);
        check_ekf_reset_total = 0;
        check_ekf_reset_count = 0;
    }
    check_ekf_reset_logger.log(duration);
}
