...
// unsigned long long fast_loop_count=0;
// unsigned long long fast_loop_total=0;
// std::chrono::high_resolution_clock::time_point fs_loop_t1;
// std::chrono::high_resolution_clock::time_point fs_loop_t2;
...
uint64_t rc_loop_t1;
uint64_t rc_loop_t2;
TimingLogger rc_loop_logger("rc_loop");

unsigned long long throttle_loop_total = 0;
unsigned long long throttle_loop_count = 0;
uint64_t throttle_loop_t1;
uint64_t throttle_loop_t2;
TimingLogger throttle_loop_logger("throttle_loop");

unsigned long long update_batt_compass_total = 0;
unsigned long long update_batt_compass_count = 0;
uint64_t update_batt_compass_t1;
uint64_t update_batt_compass_t2;
TimingLogger update_batt_compass_logger("update_batt_compass");

unsigned long long update_altitude_total = 0;
unsigned long long update_altitude_count = 0;
uint64_t update_altitude_t1;
uint64_t update_altitude_t2;
TimingLogger update_altitude_logger("update_altitude");

unsigned long long three_hz_loop_total = 0;
unsigned long long three_hz_loop_count = 0;
uint64_t three_hz_loop_t1;
uint64_t three_hz_loop_t2;
TimingLogger three_hz_loop_logger("three_hz_loop");

unsigned long long one_hz_loop_total = 0;
unsigned long long one_hz_loop_count = 0;
uint64_t one_hz_loop_t1;
uint64_t one_hz_loop_t2;
TimingLogger one_hz_loop_logger("one_hz_loop");

unsigned long long ten_hz_logging_loop_total = 0;
unsigned long long ten_hz_logging_loop_count = 0;
uint64_t ten_hz_logging_loop_t1;
uint64_t ten_hz_logging_loop_t2;
TimingLogger ten_hz_logging_loop_logger("ten_hz_logging_loop");

unsigned long long twentyfive_hz_logging_total = 0;
unsigned long long twentyfive_hz_logging_count = 0;
uint64_t twentyfive_hz_logging_t1;
uint64_t twentyfive_hz_logging_t2;
TimingLogger twentyfive_hz_logging_logger("twentyfive_hz_logging");

unsigned long long run_rate_controller_total = 0;
unsigned long long run_rate_controller_count = 0;
uint64_t run_rate_controller_t1;
uint64_t run_rate_controller_t2;
TimingLogger run_rate_controller_logger("run_rate_controller");

unsigned long long read_AHRS_total = 0;
unsigned long long read_AHRS_count = 0;
uint64_t read_AHRS_t1;
uint64_t read_AHRS_t2;
TimingLogger read_AHRS_logger("read_AHRS");
...
void Copter::rc_loop()
{
    rc_loop_t1 = AP_WCET::now_us();
    // Read radio and 3-position switch on radio
    // -----------------------------------------
    read_radio();
    rc().read_mode_switch();
    rc_loop_t2 = AP_WCET::now_us();
    uint64_t duration = rc_loop_t2-rc_loop_t1;
    rc_loop_total += duration;
    rc_loop_count += 1;
    if (rc_loop_count % 1000 == 0){
        //printf("average rc_loop measure time microseconds is %f!\n",rc_loop_total/1000.0);
        rc_loop_total = 0;
        rc_loop_count = 0;
    }
    rc_loop_logger.log(duration);
}
...
{
    throttle_loop_t1 = AP_WCET::now_us();
    ...
    throttle_loop_t2 = AP_WCET::now_us();
    uint64_t duration = throttle_loop_t2-throttle_loop_t1;
    throttle_loop_total += duration;
    throttle_loop_count += 1;
    if (throttle_loop_count % 1000 == 0){
        //printf("average throttle_loop measure time microseconds is %f!\n",throttle_loop_total/1000.0);
        throttle_loop_total = 0;
        throttle_loop_count = 0;
    }
    throttle_loop_logger.log(duration);
}
...
{
    update_batt_compass_t1 = AP_WCET::now_us();
    ...
    update_batt_compass_t2 = AP_WCET::now_us();
    uint64_t duration = update_batt_compass_t2 -update_batt_compass_t1;
    update_batt_compass_total += duration;
    update_batt_compass_count += 1;
    if (update_batt_compass_count % 1000 == 0){
        //printf("average update_batt_compass measure time microseconds is %f!\n",update_batt_compass_total/1000.0);
        update_batt_compass_total = 0;
        update_batt_compass_count = 0;
    }
    update_batt_compass_logger.log(duration);
}
...
{
    ten_hz_logging_loop_t1 = AP_WCET::now_us();
...
#endif
    ten_hz_logging_loop_t2 = AP_WCET::now_us();
    uint64_t duration = ten_hz_logging_loop_t2 -ten_hz_logging_loop_t1;
    ten_hz_logging_loop_total += duration;
    ten_hz_logging_loop_count += 1;
    if (ten_hz_logging_loop_count % 100 == 0){
        // ten_hz_logging_loop measure time microseconds is %f!\n",ten_hz_logging_loop_total/100.0);
        ten_hz_logging_loop_total = 0;
        ten_hz_logging_loop_count = 0;
        ten_hz_logging_loop_count = 0;
    }
    ten_hz_logging_loop_logger.log(duration);
}

// twentyfive_hz_logging - should be run at 25hz
void Copter::twentyfive_hz_logging()
{
    twentyfive_hz_logging_t1 = AP_WCET::now_us();
    if (should_log(MASK_LOG_ATTITUDE_FAST)) {
        Log_Write_EKF_POS();
    }
...
#endif
    twentyfive_hz_logging_t2 = AP_WCET::now_us();
    uint64_t duration = twentyfive_hz_logging_t2 -twentyfive_hz_logging_t1;
    twentyfive_hz_logging_total += duration;
    twentyfive_hz_logging_count += 1;
    if (twentyfive_hz_logging_count % 100 == 0){
       //printf("average twentyfive_hz_logging measure time microseconds is %f!\n",twentyfive_hz_logging_total/100.0);
        twentyfive_hz_logging_total = 0;
        twentyfive_hz_logging_count = 0;
        twentyfive_hz_logging_count = 0;
    }
    twentyfive_hz_logging_logger.log(duration);
}

// three_hz_loop - 3.3hz loop
void Copter::three_hz_loop()
{
    three_hz_loop_t1 = AP_WCET::now_us();
    ...
    three_hz_loop_t2 = AP_WCET::now_us();
    uint64_t duration = three_hz_loop_t2 -three_hz_loop_t1;
    three_hz_loop_total += duration;
    three_hz_loop_count += 1;
    if (three_hz_loop_count % 100 == 0){
        // three_hz_loop measure time microseconds is %f!\n",three_hz_loop_total/100.0);
        three_hz_loop_total = 0;
        three_hz_loop_count = 0;
        three_hz_loop_count = 0;
    }
    three_hz_loop_logger.log(duration);
}

// one_hz_loop - runs at 1Hz
void Copter::one_hz_loop()
{
    one_hz_loop_t1 = AP_WCET::now_us();
    if (should_log(MASK_LOG_ANY)) {
    ...
    one_hz_loop_t2 = AP_WCET::now_us();
    uint64_t duration = one_hz_loop_t2 -one_hz_loop_t1;
    one_hz_loop_total += duration;
    one_hz_loop_count += 1;
    if (one_hz_loop_count % 100 == 0){
        printf("average one_hz_loop measure time microseconds is %f!\n",one_hz_loop_total/100.0);
        one_hz_loop_total = 0;
        one_hz_loop_count = 0;
        one_hz_loop_count = 0;
    }
    one_hz_loop_logger.log(duration);
}
...
// run rate controller - runs the attitude controller at the full loop rate
void Copter::run_rate_controller()
{
    run_rate_controller_t1 = AP_WCET::now_us();
    attitude_control->rate_controller_run();
    run_rate_controller_t2 = AP_WCET::now_us();
    uint64_t duration = run_rate_controller_t2 - run_rate_controller_t1;
    run_rate_controller_total += duration;
    run_rate_controller_count += 1;
    if (run_rate_controller_count % 1000 == 0){
       // printf("average run_rate_controller measure time microseconds is %f!\n", run_rate_controller_total/1000.0);
        run_rate_controller_total = 0;
        run_rate_controller_count = 0;
    }
    run_rate_controller_logger.log(duration);
}

void Copter::read_AHRS(void)
{
    read_AHRS_t1 = AP_WCET::now_us();
    // we tell AHRS to skip INS update as we have already done it in fast_loop()
    ahrs.update(true);
    read_AHRS_t2 = AP_WCET::now_us();
    uint64_t duration = read_AHRS_t2 - read_AHRS_t1;
    read_AHRS_total += duration;
    read_AHRS_count += 1;
    if (read_AHRS_count % 1000 == 0){
        //printf("average read_AHRS measure time microseconds is %f!\n", read_AHRS_total/1000.0);
        read_AHRS_total = 0;
        read_AHRS_count = 0;
    }
    read_AHRS_logger.log(duration);
}

// read baro and log control tuning
void Copter::update_altitude()
{
    update_altitude_t1 = AP_WCET::now_us();
    ...
    update_altitude_t2 = AP_WCET::now_us();
    uint64_t duration = update_altitude_t2 -update_altitude_t1;
    update_altitude_total += duration;
    update_altitude_count += 1;
    if (update_altitude_count % 10 == 0){
        //printf("average update_altitude measure time microseconds is %f!\n",update_altitude_total/10.0);
        update_altitude_total = 0;
        update_altitude_count = 0;
        update_altitude_count=0;
    }
    update_altitude_logger.log(duration);
}
