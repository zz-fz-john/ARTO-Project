...
uint64_t gpsglitch_check_t1;
uint64_t gpsglitch_check_t2;
TimingLogger gpsglitch_check_logger("gpsglitch_check");
}
...
{
    gpsglitch_check_t1 = AP_WCET::now_us();
    // get filter status
    nav_filter_status filt_status = inertial_nav.get_filter_status();
    bool gps_glitching = filt_status.flags.gps_glitching;
    gpsglitch_check_t2 = AP_WCET::now_us();
    uint64_t duration = gpsglitch_check_t2 -gpsglitch_check_t1;
    gpsglitch_check_total += duration;
    gpsglitch_check_count += 1;
    if (gpsglitch_check_count % 100 == 0){
        //printf("average gpsglitch_check measure time microseconds is %f!\n",gpsglitch_check_total/100.0);
        gpsglitch_check_total = 0;
        gpsglitch_check_count = 0;
    }
    gpsglitch_check_logger.log(duration);
