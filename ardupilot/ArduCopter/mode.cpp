...
unsigned long long update_fight_mode_total = 0;
unsigned long long update_fight_mode_count = 0;
uint64_t update_fight_mode_t1;
uint64_t update_fight_mode_t2;
TimingLogger update_fight_mode_logger("update_fight_mode");

}
// #include "dynamic_collect.h" //zrz add
// #include <pthread.h>//zrz add
// extern pthread_key_t zrz_key;//zrz add
// #include "measure.h"
/*
...
{
    // int start_measure_zrz=2;//zrz add
    // pthread_setspecific(zrz_key,&(start_measure_zrz));//zrz add
    // dynamic_start_collecting();//zrz add
    //start_collecting();//zrz add
    


    update_fight_mode_t1 = AP_WCET::now_us();
    surface_tracking.invalidate_for_logging();  // invalidate surface tracking alt, flight mode will set to true if used

    flightmode->run();
    update_fight_mode_t2 = AP_WCET::now_us();
    uint64_t duration = update_fight_mode_t2 -update_fight_mode_t1;
    update_fight_mode_total += duration;
    update_fight_mode_count += 1;
    if (update_fight_mode_count % 1000 == 0){
       //printf("average update_fight_mode measure time microseconds is %f!\n",update_fight_mode_total/1000.0);
        update_fight_mode_total = 0;
        update_fight_mode_count = 0;
    }
    update_fight_mode_logger.log(duration);
    //end_collecting();//zrz add
    //dynamic_end_collecting(); //zrz add
}
