...
#        self.mavproxy.expect("Loaded %u (geo-)?fence" % count)
    ...
    def detect_and_handle_reboot(self, old_bootcount, required_bootcount=None, timeout=120):
        tstart = time.time()
...
#            print("t=%s" % str(t))
    ...
    def sensor_has_state(self, sensor, present=True, enabled=True, healthy=True, do_assert=False, verbose=False):
        m = self.assert_receive_message('SYS_STATUS', timeout=50, very_verbose=verbose)
