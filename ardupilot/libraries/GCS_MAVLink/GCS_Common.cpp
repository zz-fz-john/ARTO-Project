...
uint64_t GCS14update_receive_t1;
uint64_t GCS14update_receive_t2;


unsigned long long GCS14update_send_total = 0;
unsigned long long GCS14update_send_count = 0;
uint64_t GCS14update_send_t1;
uint64_t GCS14update_send_t2;
TimingLogger GCS14update_send_logger("GCS14update_send");
TimingLogger GCS14update_receive_logger("GCS14update_receive");
}
...
{
    GCS14update_send_t1= AP_WCET::now_us();
    ...
    GCS14update_send_t2 = AP_WCET::now_us();
    uint64_t duration = GCS14update_send_t2 -GCS14update_send_t1;
    GCS14update_send_total += duration;
    GCS14update_send_count += 1;
    if (GCS14update_send_count % 1000 == 0){
        //printf("average GCS14update_send measure time microseconds is %f!\n",GCS14update_send_total/1000.0);
        GCS14update_send_total = 0;
        GCS14update_send_count = 0;
    }
    GCS14update_send_logger.log(duration);
}
...
{
    GCS14update_receive_t1 = AP_WCET::now_us();
    for (uint8_t i=0; i<num_gcs(); i++) {
        chan(i)->update_receive();
    }
    // also update UART pass-thru, if enabled
    update_passthru();
    GCS14update_receive_t2 = AP_WCET::now_us();
    uint64_t duration = GCS14update_receive_t2 - GCS14update_receive_t1;
    GCS14update_receive_total += duration;
    GCS14update_receive_count += 1;
    if (GCS14update_receive_count % 1000 == 0){
       // printf("average GCS14update_receive measure time microseconds is %f!\n",GCS14update_receive_total/1000.0);
        GCS14update_receive_total = 0;
        GCS14update_receive_count = 0;
    }
    GCS14update_receive_logger.log(duration);
}
