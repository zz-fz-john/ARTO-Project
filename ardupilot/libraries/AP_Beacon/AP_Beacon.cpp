...
uint64_t AP_Beacon_update_t1;
uint64_t AP_Beacon_update_t2;
TimingLogger AP_Beacon_update_logger("AP_Beacon_update");
...
{
    AP_Beacon_update_t1 = AP_WCET::now_us();
    ...
    AP_Beacon_update_t2 = AP_WCET::now_us();
    uint64_t duration = AP_Beacon_update_t2 -AP_Beacon_update_t1;
    AP_Beacon_update_total += duration;
    AP_Beacon_update_count += 1;
    if (AP_Beacon_update_count % 1000 == 0){
        //printf("average AP_Beacon update measure time microseconds is %f!\n",AP_Beacon_update_total/1000.0);
        AP_Beacon_update_total = 0;
        AP_Beacon_update_count = 0;
    }
    AP_Beacon_update_logger.log(duration);
}
