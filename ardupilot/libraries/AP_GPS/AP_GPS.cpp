...
unsigned long long GPS_update_total = 0;
unsigned long long GPS_update_count = 0;
uint64_t GPS_update_t1;
uint64_t GPS_update_t2;
TimingLogger GPS_update_logger("GPS_update");
...
#endif
    GPS_update_t2 = AP_WCET::now_us();
    uint64_t duration = GPS_update_t2 -GPS_update_t1;
    GPS_update_total += duration;
    GPS_update_count += 1;
    if (GPS_update_count % 1000 == 0){
        //printf("average GPS_update measure time microseconds is %f!\n",GPS_update_total/1000.0);
        GPS_update_total = 0;
        GPS_update_count = 0;
    }
    GPS_update_logger.log(duration);
}
