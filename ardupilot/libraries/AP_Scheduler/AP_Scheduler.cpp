...
uint64_t vehical_loop_t1;
uint64_t vehical_loop_t2;
TimingLogger vehical_loop_logger("vehical_loop");
 ...
 */
extern "C" {
   volatile int record_time_flag = 0; // 强制使用 C 语言符号，这样 .c 文件才能真正访问到它
}
// unsigned long long time_array[25000];
double time_array[25000];
uint64_t timestamp_array[25000]; // 存储每个样本的时间戳（相对于开始时间，单位：微秒）
int time_index=0;
uint64_t record_start_time_us = 0; // 记录开始采集时间
const uint64_t RECORD_DURATION_MS = 25000; // 采集时长：25000毫秒
void AP_Scheduler::run(uint32_t time_available)
{
    ...
    vehical_loop_t1 = AP_WCET::now_us();//zrz add to measure overhead
    ...
    vehical_loop_t2 = AP_WCET::now_us();
    ...
    uint64_t duration = vehical_loop_t2 -vehical_loop_t1;
    ...
    if (vehical_loop_count % 1000 == 0){
        ...
        //     fclose(fp);
        // }
       // printf("average main_loop measure time microseconds is %f!\n",vehical_loop_total/1000.0);
        vehical_loop_total = 0;
        vehical_loop_count = 0;
    }
    vehical_loop_logger.log(duration);
    if(record_time_flag==1){
        // 第一次进入时，记录开始时间
        if (time_index == 0) {
            record_start_time_us = AP_HAL::micros();
        }
        
        // 采集数据和时间戳
        if (time_index < 25000) {
            uint64_t current_time_us = AP_HAL::micros();
            uint64_t elapsed_us = current_time_us - record_start_time_us;
            
            time_array[time_index] = duration_us;
            timestamp_array[time_index] = elapsed_us;  // 记录相对于开始时间的微秒数
            time_index++;
        }
        ...
        if (elapsed_ms >= RECORD_DURATION_MS || time_index >= 25000) {
            const char* filepath = "/tmp/main_loop_time.txt";
            printf("Attempting to write file to %s...\n", filepath);
            printf("Collected %d samples in %llu ms\n", time_index, elapsed_ms);
            
            // 使用 C 语言标准 IO (fopen/fprintf)
            FILE *fp = fopen(filepath, "w");
            
            if (fp) {
                // 写入文件头，说明列的含义
                fprintf(fp, "# timestamp_us(microseconds) loop_time_us(microseconds)\n");
                
                for (int i = 0; i < time_index; i++){
                    // 格式：时间戳(微秒) 循环时间(微秒)
                    fprintf(fp, "%llu %.6f\n", timestamp_array[i], time_array[i]);
                }
                fclose(fp);
                printf("main_loop_time.txt write success! Total: %d samples\n", time_index);
            } else {
                printf("Failed to open file! Error: %s\n", strerror(errno));
            }
            record_time_flag=0;
            time_index=0;
            record_start_time_us = 0;
        }
    }
