...
uint64_t AP_BattMonitor_read_t1;
uint64_t AP_BattMonitor_read_t2;
TimingLogger AP_BattMonitor_read_logger("AP_BattMonitor_read");
...
{
    AP_BattMonitor_read_t1= AP_WCET::now_us();
    ...
    checkPoweringOff();
    AP_BattMonitor_read_t2 = AP_WCET::now_us();
    uint64_t duration = AP_BattMonitor_read_t2 -AP_BattMonitor_read_t1;
    AP_BattMonitor_read_total += duration;
    AP_BattMonitor_read_count += 1;
    if (AP_BattMonitor_read_count % 1000 == 0){
        //printf("average AP_BattMonitor_read measure time microseconds is %f!\n",AP_BattMonitor_read_total/1000.0);
        AP_BattMonitor_read_total = 0;
        AP_BattMonitor_read_count = 0;
    }
    AP_BattMonitor_read_logger.log(duration);
}
