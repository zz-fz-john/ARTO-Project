...
unsigned long long AP_Baro_update_total = 0;
unsigned long long AP_Baro_update_count = 0;
uint64_t AP_Baro_update_t1;
uint64_t AP_Baro_update_t2;
TimingLogger AP_Baro_update_logger("AP_Baro_update");
unsigned long long baro_accumulate_total = 0;
unsigned long long baro_accumulate_count = 0;
uint64_t baro_accumulate_t1;
uint64_t baro_accumulate_t2;
TimingLogger baro_accumulate_logger("baro_accumulate");
...
{
    AP_Baro_update_t1= AP_WCET::now_us();
...
#define MASK_LOG_ANY                    0xFFFF
    ...
    AP_Baro_update_t2 = AP_WCET::now_us();
    uint64_t duration = AP_Baro_update_t2 -AP_Baro_update_t1;
    AP_Baro_update_total += duration;
    AP_Baro_update_count += 1;
    if (AP_Baro_update_count % 1000 == 0){
        printf("average AP_Baro_update measure time microseconds is %f!\n",AP_Baro_update_total/1000.0);
        AP_Baro_update_total = 0;
        AP_Baro_update_count = 0;
    }
    AP_Baro_update_logger.log(duration);
}
...
{
    baro_accumulate_t1 = AP_WCET::now_us();
    for (uint8_t i=0; i<_num_drivers; i++) {
        drivers[i]->accumulate();
    }
    baro_accumulate_t2 = AP_WCET::now_us();
    uint64_t duration = baro_accumulate_t2 -baro_accumulate_t1;
    baro_accumulate_total += duration;
    baro_accumulate_count += 1;
    if (baro_accumulate_count % 1000 == 0){
        printf("average baro_accumulate measure time microseconds is %f!\n",baro_accumulate_total/1000.0);
        baro_accumulate_total = 0;
        baro_accumulate_count = 0;
    }
    baro_accumulate_logger.log(duration);
}
