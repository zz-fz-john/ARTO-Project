...
python3 binary_rewrite.py --binary_path ${ROOT_DIR}/oat-evaluation/"$1"/build/"$1" --disassembly_file_name ${ROOT_DIR}/oat-evaluation/"$1"/build/"$1".S --only_called_once_func_file ../output/only_called_once_func_backup_"$1".txt --ToInsertFuncFile ../output/ToInsertFunc_"$1".txt --output_binary_path ../output/"$1"_output || { echo "Binary rewrite failed"; exit 1; }
